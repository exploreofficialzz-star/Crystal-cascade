...
# BUG-001 FIX: replace queue_free() with immediate remove+free so that
# _connect_tubes() and _sync_visuals() see ONLY the newly-added Tube3D nodes.
# queue_free() defers deletion to end-of-frame; calling get_child_count()
# in the same frame returns old + new count, causing signal/crystal
# connections to land on nodes that are about to be deleted.
