...
func setup(index: int, cap: int) -> void:
	...
	# NOTE: The tube Area3D handles input for EMPTY tubes (no crystal on top).
	# Crystal3D areas handle input when crystals are present.
	# main.gd debounces both paths so only one _on_tube_tapped fires per tap.
	var area := Area3D.new()
