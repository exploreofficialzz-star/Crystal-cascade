...
# ── UI size constants (base viewport 1080 × 1920) ─────────────────────────────
# All sizes are in logical pixels matching the Godot viewport.
# canvas_items stretch scales everything proportionally to the real screen.
const MRG   := 44      # outer safe-area margin
const SEP   := 18      # default item separation
const W_BOX := 980     # content box min-width (of 1080 viewport)

const F_TITLE  := 78   # screen title
const F_SUB    := 34   # subtitle / caption
const F_BTN_H  := 46   # hero CTA font
const F_BTN    := 38   # standard button font
const F_BTN_S  := 32   # small / back button font
const F_STAT   := 38   # stats / balance labels
const F_HUD_L  := 40   # HUD large line
const F_HUD_S  := 34   # HUD small line
const F_CARD   := 32   # level-select card
const F_BODY   := 34   # body text / instructions
const F_FOOT   := 26   # footer / tiny labels
const F_TOAST  := 34   # toast notification

const H_HERO   := 142  # Play / primary CTA
const H_LG     := 118  # secondary important actions
const H_BTN    := 106  # standard buttons
const H_SM     := 88   # back / minor actions
const H_CTRL   := 100  # in-game control bar
const H_CARD   := 160  # level select cards
# ─────────────────────────────────────────────────────────────────────────────
...
# BUG-002 FIX: Both Crystal3D and Tube3D areas emit input for the same tap.
# Track the last tube+frame so the second call in the same engine frame is dropped.
...
func _button(label: String, action: Callable,
	...
	b.add_theme_stylebox_override("normal",  _btn_style(n))
	b.add_theme_stylebox_override("hover",   _btn_style(h))
	b.add_theme_stylebox_override("pressed", _btn_style(p))
	b.pressed.connect(action)
	return b

func _label(text: String, fsize: int, color := Color("#eef1ff"),
...
func _content_scroll() -> VBoxContainer:
	# Returns a scrollable full-screen content column, centred horizontally.
	...
	mrg.add_theme_constant_override("margin_top",    60)
	mrg.add_theme_constant_override("margin_bottom", 60)
	center.add_child(mrg)
	var box := VBoxContainer.new()
	box.custom_minimum_size = Vector2(W_BOX, 0)
...
func show_home() -> void:
	...
	box.add_child(_label("by chAs  •  Godot 3D Edition", F_FOOT, Color("#4a5480")))
...
func show_levels() -> void:
	...
	for id in range(1, max_display + 1):
		...
		card.add_theme_font_size_override("font_size", F_CARD)
		if locked:
			card.text     = "🔒\n%d" % id
			card.disabled = true
			...
			card.text = "%d\n%s" % [id,
				"★".repeat(nstars) + "☆".repeat(3 - nstars)]
			# CRASH FIX: use .bind(id) to capture the VALUE of id, not a
			# reference to the loop variable.  Without .bind(), all lambdas
			# in a GDScript for-loop share the same reference and every card
			# ends up calling start_level(max_display) after the loop ends.
...
func _make_board() -> void:
	...
	# world.arrange() uses immediate free() — see world.gd BUG-001 fix
...
func _sync_visuals() -> void:
	# BUG-004 FIX: iterate tubes.size() — the authoritative source of truth.
	for i in range(tubes.size()):
		var tube_node := world.get_tube(i)
		if not tube_node: continue

		# BUG-005 FIX: immediate free so a same-frame re-sync doesn't see
		# stale crystals alongside freshly-created ones.
		...
		for j in range(values.size()):
			...
			gem.setup(str(values[j]),
				...
				float(i * 17 + j), j)
			# Use a single-param lambda matching the signal signature exactly.
			# Capture tube index i via default parameter — safest in GDScript 4.
...
# ── TUBE / CRYSTAL INTERACTION ────────────────────────────────────────────────

# BUG-002 FIX: per-frame debounce prevents the double-call that occurs when
# both a Crystal3D area and its parent Tube3D area emit input for the same tap.
...
func show_pause() -> void:
	...
	box.add_child(_button("▶  RESUME",
		func(): ov.queue_free(); game_status = GameData.Status.PLAYING,
		H_LG, F_BTN_H, true))
	box.add_child(_button("↺  RESTART",
		func(): ov.queue_free(); start_level(current_level), H_BTN, F_BTN))
	box.add_child(_button("LEVELS",
		func(): ov.queue_free(); show_levels(), H_BTN, F_BTN))
	box.add_child(_button("HOME",
		func(): ov.queue_free(); show_home(), H_BTN, F_BTN))
...
func _show_tutorial() -> void:
	...
	inst.text = (
		...
		+ "2. Tap a tube with the same color at the top, or an empty tube.\n"
		...
		+ "Use the CAM button to rotate the camera around the board.")
	...
	box.add_child(_button("GOT IT  ▶",
		func(): ov.queue_free(); tutorial_visible = false
		        save.data.tutorial = true; save.save(),
		H_LG, F_BTN, true))
...
func _toast(msg: String) -> void:
	...
	# Anchor to bottom-wide, offset upward above the control bar
